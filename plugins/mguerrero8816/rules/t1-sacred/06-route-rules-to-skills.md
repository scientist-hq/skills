...
When the user asks you to save, add, or remember a rule, skill, or preference — ALWAYS write it to the correct file in `.claude/skills/` (skills) or `.claude/rules/` (rules). NEVER use the auto-memory system.
...
- **ALWAYS** write to the correct file in `.claude/skills/` or `.claude/rules/` — use the `authoring` skill for guidance on where things go
