...
Use the current branch context and any stated intent to determine the PR type, then load the appropriate specialized skill after completing the rules below:
...
| Standard feature/fix PR into `main` | No additional skill needed |

## Step 2: Apply Universal PR Rules
...
1. **Ticket URL passed as argument** — use it directly
2. **Ticket clear from conversation context** (user mentioned it, branch name contains an issue number, etc.) — confirm with the user before proceeding: "I'm going to link this to #XXXXX — is that right?"
3. **Neither** — stop and ask the user for a ticket URL before doing anything else. Do not proceed without one unless the user explicitly says there is no ticket.

#### Step 2: Verify the ticket matches the branch

Once the ticket is identified:

1. Fetch the ticket body: `gh issue view <number> --json title,body`
2. Get the branch diff summary: `git diff <parent-branch>...HEAD --stat`
3. Skim both — does the work on the branch look like it addresses what the ticket describes?

**If they match** → proceed normally.

**If they don't look related** → warn the user before continuing:
> "The ticket (#XXXXX — '[title]') doesn't seem to match the changes on this branch ([summarise the mismatch briefly]). Do you want to proceed anyway?"

Only continue if the user confirms. Do not open the PR silently if there is a mismatch.

#### Step 3: Reference the ticket in the PR

- Use `"Resolves #XXXXX"` — closes the ticket automatically when the PR merges
- Only use `"As part of #XXXXX"` if the user explicitly says the ticket should stay open after merge
- ❌ NEVER write a PR description with no issue reference
