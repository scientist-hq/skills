...
**🚨 CRITICAL: ALWAYS run `/pr-rules` when interacting with ANY pull request - whether creating, editing, reviewing, or updating. This is MANDATORY and cannot be skipped. 🚨**

This applies to:
- Creating new pull requests
- Editing existing pull request descriptions
- Adding comments to pull requests
- Updating pull request testing instructions
- Reviewing pull requests
- ANY interaction with a pull request

**Workflow for Creating Pull Requests:**
1. **ALWAYS run `/pr-rules` first** - this loads universal PR rules including draft mode requirement
2. When the user asks to open/create a pull request, determine the type:
3. **If it's a hotfix**, run `/open-hotfix-pr` (which provides hotfix-specific instructions)
4. **If it's a panel-to-card migration**, run `/open-panel-card-pr` (which provides panel migration specific instructions)
5. **If it's a Bootstrap 5 migration**, run `/open-bs5-pr` (which provides additional BS5-specific instructions)
6. **If it's removing unused/unneeded code**, run `/open-cleanup-pr` (which provides cleanup PR specific instructions)
7. **If the type is unclear or no specific slash command applies**, read `/Users/mike/rx/.claude/commands/pr-rules.md` directly and use it as the sole guide for creating the PR

**PR Area Labels - CRITICAL RULE:**
- **Storefront and Backoffice labels are MUTUALLY EXCLUSIVE**
- **NEVER** add both labels to the same PR
- A PR affects EITHER backoffice OR storefront, never both
- If you accidentally add both, immediately remove the incorrect one

**PR Title Prefixes - CRITICAL RULES:**
- **ALL hotfix PRs** MUST have titles prefixed with "HOTFIX:" (all caps)
- **Example**: "HOTFIX: Fix nil object error when creating new provider inventory catalog"
- **Example**: "HOTFIX: Restore missing purchase order display"
- This prefix is MANDATORY for all hotfixes and cannot be omitted

**Hotfix Base Branch - CRITICAL RULE:**
- **ALL hotfix PRs MUST target the `production` branch**
- Use `--base production` when creating hotfix PRs
- Hotfixes are urgent production fixes and ALWAYS go directly to production, never to main

**Staging PRs - CRITICAL RULE:**
- **ALWAYS run `/open-staging-pr`** when opening a PR that targets the `staging` branch
- Cues that indicate a staging PR:
  - User says "open a staging PR" or "PR to staging"
  - Current branch name contains `staging`
  - User mentions the PR should go to staging instead of main

**How to detect panel-to-card migrations:**
- Branch name contains: `panel`, `panel-to-card`, `cards`, or `panel-card`
- Git diff shows class changes: `.panel` → `.card`, `.panel-heading` → `.card-header`, `.panel-body` → `.card-body`, `.panel-footer` → `.card-footer`
- JavaScript files updating panel selectors (e.g., `$('.panel')` → `$('.card')`)
- HAML/ERB views replacing panel classes with card classes
- Changes to jQuery plugins (matchHeight, sortable) targeting panel classes

**How to detect Bootstrap 5 migrations:**
- Branch name contains "bs5" (e.g., `mg12-legacy-provider-bills-bs5`)
- Changed files include views with Bootstrap 5 class updates
- Controller adds `layout :backoffice_bs5_layout` or `layout :storefront_bs5_layout`
- Removing Bootstrap 3 classes (panel, form-inline, etc.) and replacing with Bootstrap 5 (card, d-flex, etc.)
