...
#
# Posting default: post when the PR author is rranauro (self-review), write to
# file otherwise. We never post to a colleague's PR without an explicit --post —
# outward-facing comments on someone else's work are Ron's call, not the tool's.
...
if [ "$POST" = "auto" ]; then
  if [ "$AUTHOR" = "$ME" ]; then POST="yes"; else POST="no"; fi
fi
...
# --- Delivery -----------------------------------------------------------------
OUT_DIR="$HOME/dev/scientist/rx/tmp/reviews/pr-${PR_NUM}"
...
if [ "$POST" = "yes" ]; then
  ...
  DELIVERY="Do NOT post anything to GitHub. Write the review to \`${OUT_FILE}\` (\`mkdir -p\` the directory first). This PR is authored by someone else; posting is Ron's decision, not yours."
fi

# --- Prompt -------------------------------------------------------------------
# Written for a senior Rails reader. Name the specific instance and why it
# matters here; no tutorials on what an N+1 or a callback is.
...
Review principles:
- **Anchor every finding to the PR's intent** (author description + ticket ACs). RX is massive and side-effect bugs always exist; concerns outside this PR's intent flood the signal. One outside the intent is at most a one-line question.
...
- **Check the diff against the repo's documented conventions** in CLAUDE.md and rx/.claude/CLAUDE.md — cite the specific rule when the diff breaks one. Review the change, not the codebase.
...
The HTML comment is a stable marker that /rranauro:review-copilot uses to find this review (it is posted under Ron's gh user, not a bot login, so login filtering does not work). Do not omit or alter it.
