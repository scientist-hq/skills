...
Confirm the worktree path via `git worktree list` — it should be a sibling of the home checkout, `{{repo.workspace_root}}/{{repo.worktree_prefix}}-<branch-name>/`. If it isn't listed there, surface what you found and ask the user before proceeding.
...
- `git -C {{repo.workspace_root}}/{{repo.worktree_prefix}}-<branch-name> status --porcelain`
...
- `git worktree remove {{repo.workspace_root}}/{{repo.worktree_prefix}}-<branch-name>`
